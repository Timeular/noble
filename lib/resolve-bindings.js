var os = require('os');
var bleAdapter = require('ble-adapter')

function recentEnoughVersion() {
  const ver = os.release().split('.').map(Number);
  return (ver[0] > 10 ||
         (ver[0] === 10 && ver[1] > 0) ||
         (ver[0] === 10 && ver[1] === 0 && ver[2] >= 17134))
}

function hasBLEAdapter() {
  return bleAdapter.list().findIndex(function(adapter) {
    return adapter && adapter.bleCapable && adapter.isConnected
  }) != -1
}
...
module.exports = function() {
  ...
  if (process.env.NOBLE_WEBSOCKET) {
    ...
    if(!process.env.NOBLE_FORCE_DONGLE && recentEnoughVersion() && hasBLEAdapter()) {
